...
function updateChipDisplay() {
  ...
  el('chipIndex').textContent = `${state.currentChipIdx + 1} / ${state.chips.length}`;
  el('chipName').textContent = chip.id;
  el('btnPrev').disabled = state.currentChipIdx === 0;
  el('btnNext').disabled = state.currentChipIdx === state.chips.length - 1;
...
async function runInference() {
  if (!state.chips.length || state.busy) return;
...
function bindButtons() {
  el('btnPrev').addEventListener('click', prevChip);
  el('btnNext').addEventListener('click', nextChip);
